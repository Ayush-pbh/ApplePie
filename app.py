...
hands = mpHands.Hands(max_num_hands=1)             #default values are good for right now
...
while True:
    ...
    if results.multi_hand_landmarks:
        for handLms in results.multi_hand_landmarks:
            index_A = (int(handLms.landmark[8].x),int(handLms.landmark[8].y))
            cv2.circle(img, index_A, 5, (255,255,255), 5)
